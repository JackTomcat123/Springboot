...
<body>
	...
	<div style="width: 100%; height: 500px;">
		<div style="width: 104px; height: 100%;float: left; margin-left: 10px;">
			...
			<c:forEach items="${list }" var="l">
				<p style="font-size: 12px;">
			<a href="javascript:void(0)" onclick="GO(${l.className})">${l.className}</a>
			</p>
			</c:forEach> --%>
			<div>
				<strong>班级学生：</strong>
 ...
 <script type="text/javascript">
	 ...
	 function loadStus() {
		...
		$.getJSON("../../stus?classId="+'<%=request.getParameter("classId")%>'+"&tid="+$("#tid").text().trim(), function(json) {
				if(json == "TID-NULL") {
					$("#studentExamCount").html("<h1>请登录后再查看</h1>");
					alert("请登录后再查看");
				} else {
	 ...
	 var stuExamInfo = echarts.init(document.getElementById("stuExamInfo"), "shine");
		...
		function loadShopLine() {
			...
			$.getJSON("../../avgcounts?classId="+'<%=request.getParameter("classId")%>'+"&tid="+$("#tid").text().trim(), function(json) {
				if(json == "TID-NULL") {
